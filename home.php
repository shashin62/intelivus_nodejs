...
//        $rs = mysqli_query($db, $qr) or die("cannot select projects main" . mysqli_error($db));
...
//            $rnrf = $row["nrf"];
//            $onhold = $records - $rsave - $qasubmit - $rreject - $rcomplete;
//        }
...
//        $trnrf = 0;
//
//        $qr = "select * from allocat where userid='" . $_SESSION["sadmin_subid"] . "' and activate = 1 and proid='" . $proid . "'";
//        $rs = mysqli_query($db, $qr) or die("cannot select projects" . mysqli_error($db));
//        while ($row = mysqli_fetch_array($rs)) {
//
//            $trecords = $trecords + ($row["rend"] - $row["rstart"] + 1);
//            $tsave = $tsave + $row["rsave"];
//            $tcomplete = $tcomplete + $row["complete"];
//            $treject = $treject + $row["qareject"];
//            $tqasubmit = $tqasubmit + $row["rqacount"];
//            $trnrf = $trnrf + $row["nrf"];
//        }
//
//        $records = $trecords;
...
//    }
    ...
    $sql = "select status from data where proid='" . $proid . "'";
    $result = mysqli_query($db, $sql) or die("cannot select projects" . mysqli_error($db));
    ...
    ?>
                                ...
                                <div class="smallboxy">
                                    <a href="<?php echo 'home.php?type=1' . $psql; ?>"><div class="fbox" style="background:#009BBA; margin-left:5px;"><span class="absbox"><i class="fa fa-save"></i></span> <h2><?php echo $rsave; ?></h2><div class="tip" >Saved</div></div></a>

                                    <a href="<?php echo 'home.php?type=2' . $psql; ?>"><div class="fbox" style="background:#78216F;"><span class="absbox"><i class="fa fa-paste"></i></span><h2><?php echo $qasubmit; ?></h2><div class="tip">Submit To QA</div></div></a>


                            <a href="<?php echo "home.php?type=0" . $psql;?>"><div class="fbox" style="background:#3F9C34;"><span class="absbox"><i class="fa fa-copy"></i></span><h2><?php echo $onhold; ?></h2><div class="tip">WIP</div></div></a>

                                    <a href="<?php if ($user_type_admin == "Main Rights") {
                                echo 'home.php?type=3' . $psql;
                            } else {
                                echo "#";
                            } ?>"><div class="fbox" style="background:#36C4B6;"><span class="absbox"><i class="fa fa-thumbs-up"></i></span><h2><?php echo $rcomplete; ?></h2><div class="tip">Complete</div></div></a>
                                    <a href="<?php echo 'home.php?type=4' . $psql; ?>"><div class="fbox" style="background:#1D9E75;"><span class="absbox"><i class="fa fa-remove"></i></span><h2><?php echo $rreject ?></h2><div class="tip">Rejected</div></div></a>

                                    <a href="<?php echo 'home.php?type=5' . $psql; ?>"><div class="fbox" style="background:#FF3300;  margin-right:0px;"><span class="absbox"><i class="fa fa-cross"></i></span><h2><?php echo $rnrf; ?></h2><div class="tip">Record Not Found</div></div></a>
    ...
    ?>
                                        ...
                                        if ($user_type_admin == "Sub Rights" && $user_qa_rights != 1) {
                                            $str = "select * from allocat where userid='" . $_SESSION["sadmin_subid"] . "' and activate=1  and proid='" . $proid . "'";
                                            $srsr = mysqli_query($db, $str) or die("cannot Select" . mysqli_error($db));
                                            $numrow = mysqli_num_rows($srsr);
                                            $allocsql = "allocid in (";
                                            while ($srarray = mysqli_fetch_array($srsr)) {
                                                $sp = $sp . " " . $srarray["id"] . " ,";
                                            }
                                            if ($numrow == 0) {
                                                $sp = "-1,";
                                            ...
                                            $sp = rtrim($sp, ",");
                                            $allocsql = $allocsql . $sp . ")";
                                            $sql = $sql . " and " . $allocsql;
                                        }
                                        if ($rtype == 5) {
                                            if ($proid == "") {
                                                $proid = 0;
                                            }
                                            $sql = "where proid=" . $proid . " and stan_comments='RECORD NOT FOUND'";
                                            $allocsql = "";
                                            if ($user_type_admin == "Sub Rights" && $user_qa_rights != 1) {
                                                $str = "select * from allocat where userid='" . $_SESSION["sadmin_subid"] . "' and activate=1  and proid='" . $proid . "'";
                                                $srsr = mysqli_query($db, $str) or die("cannot Select" . mysqli_error($db));
                                        ...
                                        $c = 0;
                                        $qy = "select * from data where proid=" . $proid . " order by cid";
                                        $per_page = 10;
